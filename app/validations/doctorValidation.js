...
const { getDoctorByEmail } = require("../services/doctorService");
const DoctorVerification = require("../models/DoctorVerification");
const Doctor = require("../models/Doctor");

const doctorSignupValidator = () => {
  return [
    //Check that email isn't taken
    check("email")
      .custom(async (value) => {
        let doctorExist = await getDoctorByEmail(value);

        if (doctorExist[0] !== false) {
          // Email exists in doctor collection
          // Check if it's verified in doctorverifications
          const verification = await DoctorVerification.findOne({ email: value });
          ...
          if (verification) {
            ...
            if (expiresAt > currentTime) {
              ...
              // Verification has expired, delete the verification and Doctor and return success
              await DoctorVerification.deleteOne({ email: value });
              await Doctor.deleteOne({ email: value });
              return true;
            }
          } else {
    ...
    body("specialty", "Please enter your Specialty ").trim().notEmpty(),
